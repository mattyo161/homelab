...
## Why blkid sees DATA partitions that ansible_facts does not (GPT PARTLABEL vs FS label)

`ansible_facts.devices[dev].partitions[part].links.labels` contains **filesystem labels** — the label embedded inside the filesystem itself, set at format time (e.g. `mkfs.exfat -L DATA1`, `mkfs.ext4 -L DATA1`). This is what tools like `e2label`, `exfatlabel`, and `lsblk -o LABEL` read.

**GPT partition table labels** (also called partition names) are a completely different field stored in the GPT header entry for that partition, set by partitioning tools (`parted name 6 DATA1`, `gdisk` partition name). They appear as `PARTLABEL=` in `blkid` output but are **not** surfaced in `ansible_facts` because the `setup` module reads `/sys/block` device metadata (which exposes FS labels via udev symlinks in `/dev/disk/by-label/`) rather than parsing the raw GPT header.

Summary of the two label types:

| Type | Where stored | Set by | Visible in ansible_facts | blkid field |
|------|-------------|--------|--------------------------|-------------|
| Filesystem label | Inside the filesystem | `mkfs -L` / `e2label` / `exfatlabel` | Yes — `links.labels` | `LABEL=` |
| GPT partition label | GPT partition entry | `parted name=` / `gdisk` | No | `PARTLABEL=` |

`mou-mini4` was partitioned with GPT partition names (`PARTLABEL=DATA1/2/3`) but the exfat filesystems on those partitions were not given a matching FS label (`LABEL=`). Running `exfatlabel /dev/sda6 DATA1` on each partition would add the FS label and make them visible in `ansible_facts` without needing `blkid` at all.

## Future: Longhorn storage classes by DATA partition label

The DATA partition labels (`DATA1`, `DATA2`, `DATA3`) provide a natural way to segment storage capacity by purpose across the cluster. Some ideas to explore:

- **`DATA1` → general-purpose Longhorn storage** — default data path used by most PVCs
- **`DATA2` → dedicated for Prometheus / log retention** — isolates high-write workloads so they don't contend with application data
- **`DATA3` → backup / snapshot target** — could be used by Longhorn backup jobs or a separate backup tool

Longhorn supports multiple data paths via **node-level disk configuration** (Longhorn settings → nodes → add disk). Each disk can be tagged with custom labels (e.g. `storage-class: prometheus`) and Longhorn `StorageClass` objects can use `diskSelector` / `nodeSelector` to pin volumes to specific tagged disks.

The storage Ansible role already mounts each partition at `/mnt/data/<LABEL>`. The next step would be a task that registers each mount with the Longhorn API (or Longhorn `Node` CR) with the appropriate tag. This is deferred — worth a DDD when the storage topology is finalized.

## TODO: upstream PR — k3s-ansible raspberrypi role check-mode false positive

**Repo:** `k3s-io/k3s-ansible` (and fork `mattyo161/k3s-ansible`, branch `mou-latest`)

**Problem:** In `--check --diff` mode, `ansible.builtin.command` tasks are skipped and Ansible returns a placeholder result with `rc` unset or `0`. The two Pi detection greps in `roles/raspberrypi/tasks/main.yml` therefore appear to succeed on every host in check mode, causing the `Run Raspberry Pi-specific tasks` block to be entered on all Ubuntu hosts — including x86_64 Mac minis. `Ubuntu.yml` then tries to write to `/boot/firmware/cmdline.txt` (or falls back to `/boot/firmware/current/cmdline.txt`), which doesn't exist on non-Pi hardware, causing the run to fail.

In a normal (non-check) run the greps correctly return `rc=1` on non-Pi hosts and the block is skipped.

**Fix applied to fork:** `roles/raspberrypi/tasks/prereq/Ubuntu.yml` — added `when: raspberrypi_boot_file.stat.exists` to the cgroup `replace` task. `ansible.builtin.stat` runs correctly in check mode and reports the file as absent on non-Pi hardware, so the task is safely skipped. The broken ternary fallback to `/boot/firmware/current/cmdline.txt` was also removed.

**To upstream:** open a PR against `k3s-io/k3s-ansible` with the same one-line `when:` addition. The same fix should be reviewed for `Debian.yml` and `Raspbian.yml` which have the same `replace` task pattern (though they fall back to `/boot/cmdline.txt` rather than `/boot/firmware/current/cmdline.txt`, so they fail differently on non-Pi hardware in check mode).